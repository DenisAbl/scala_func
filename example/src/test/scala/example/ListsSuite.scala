 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of the one element list") {
    assert(sum(5::Nil) === 5)
  }

  test("sum of a few numbers:negative and positive") {
    val testList = List(1,-26,87,999,-100,0)
    assert(sum(testList) === testList.sum)
  }

  test("sum of zeros list") {
    val testList = List(0,0,0,0,0,0)
    assert(sum(testList) === 0)
  }

  test("max for empty list should throw NoSuchElementException"){
    intercept[NoSuchElementException]{
      max(Nil)
    }
  }

  test("max of a few numbers: negative and positive") {
    val testList = List(1,-26,87,999,-100,0)
    assert(max(testList) === testList.max)
  }

  test("max of a few numbers: with duplicates") {
    val testList = List(1,-26,55,55,87,-55,999,-100,0,55,0,43)
    assert(max(testList) === testList.max)
  }

  test("max of a duplicates only list") {
    val testList = List(-4,-4,-4,-4,-4,-4,-4)
    assert(max(testList) === testList.max)
  }

  test("max of negative list"){
    val testList = List(-99,-4,-1,-76,-45,-44,-14)
    assert(max(testList) === testList.max)
  }
